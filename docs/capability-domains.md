...
Dispatch results use explicit outcomes: `handled`, `transformed`, `denied`, `failed`, `degraded`, `short-circuited`, `overridden`, `no-owner`, `no-handler`, `unsupported-command`, and `incompatible-version`. No-owner, no-handler, unsupported-command, and incompatible-version decisions are recorded in diagnostics so support bundles explain why nothing happened.
